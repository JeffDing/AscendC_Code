...
class KernelSinh {
 public:
  ...
  __aicore__ inline void Init(GM_ADDR x, GM_ADDR out,
    ...
    pipe.InitBuffer(inQueueIN, BUFFER_NUM, this->tileLength * sizeof(half));
 ...
 private:
  ...
  __aicore__ inline void Compute(int32_t progress) {
    ...
    Reciprocal(outLocal, xLocal, this->tileLength);
    Sub(outLocal, xLocal, outLocal, this->tileLength);
    half scalar = 0.5;
    Muls(outLocal, outLocal, scalar, this->tileLength);
...
extern "C" __global__ __aicore__ void sinh_custom(GM_ADDR x, GM_ADDR out,
                                                     GM_ADDR workspace,
                                                     GM_ADDR tiling) {
  ...
  op.Init(x, out, tiling_data.value, tiling_data.blockLength,
...
void sinh_custom_do(uint32_t blockDim, void* l2ctrl, void* stream,
                       uint8_t* x, uint8_t* out,
                       uint8_t* workspace, uint8_t* tiling) {
  sinh_custom<<<blockDim, l2ctrl, stream>>>(x, out, workspace, tiling);
}
#endif
