...
TEST_F(ReluKernelTest, test_kernel_run)
{
    ...
    // tilingData 字段与 op_kernel/relu_tiling_data.h 结构一致：
    //   {totalNum, blockFactor, ubFactor}，此处单 block 单 tile 跑全量数据
    // ubFactor 取 8192：单 buffer 32KB，双 buffer 64KB，两队列合计 128KB，
    // 远小于 UB 上限（192KB），留足余量给 TPipe 内部开销
    ReluTilingData* tilingData = reinterpret_cast<ReluTilingData*>(tiling);
    tilingData->totalNum = size;
    tilingData->blockFactor = size;
    tilingData->ubFactor = 8192;

    // tilingKey=1 对应 RELU_TPL_SCH_MODE_1（float），与 op_host tilingKey 逻辑一致
