...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    int64_t totalNum = inputStorageShape->GetOriginShape().GetShapeSize();
    ...
    int64_t dataPerBlock = 32 / typeSize;
    int64_t totalBlocks = CeilDiv(totalNum, dataPerBlock);
    int64_t blocksPerCore = CeilDiv(totalBlocks, coreNum);
    int64_t blockFactor = blocksPerCore * dataPerBlock;
    if (blockFactor <= 0) {
        blockFactor = dataPerBlock;
    }

    int64_t ubFactor = FloorAlign(static_cast<int64_t>(ubSize / (typeSize * bufferNum * 2)), dataPerBlock);
    if (ubFactor <= 0) {
        ubFactor = dataPerBlock;
    }
